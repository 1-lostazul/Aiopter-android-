...
class BackendClient(baseUrl: String, private val tokenVault: TokenVault) {
    private val baseUrl = EndpointPolicy.requireAllowed(baseUrl, BuildConfig.DEBUG)
                    ...
                    try {
                        response.use {
                            if (!response.isSuccessful) {
                                ...
                                onEvent(StreamEvent.Error(detail?.takeIf(String::isNotBlank) ?: "AIopter could not reach the assistant."))
                                return@use
                            }
