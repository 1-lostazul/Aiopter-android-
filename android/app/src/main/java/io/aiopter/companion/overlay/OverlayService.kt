...
import io.aiopter.companion.ai.ProposedAction
...
class OverlayService : Service(), RecognitionListener {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_START_VOICE -> startVoiceRecognition()
            ACTION_STOP_ALL -> stopEverything()
        }
    ...
    private fun buildPanel(): View {
        ...
        header.addView(TextView(this).apply { text = "  AIOPTER"; setTextColor(Color.WHITE); textSize = 18f; typeface = android.graphics.Typeface.DEFAULT_BOLD }, LinearLayout.LayoutParams(0, dp(48), 1f))
    ...
    private fun showAccessConfirmation(appLabel: String, packageName: String) {
        if (packageName.isBlank()) {
            showError("AIopter could not identify the foreground app, so no screen context was sent.")
            return
        }
    ...
    private fun openApprovedAction(action: ProposedAction) {
        AlertDialog.Builder(this).setTitle("Open another app?").setMessage("AIopter prepared “${action.label}”. Nothing happens until you approve.")
    ...
    private fun notification(): Notification = NotificationCompat.Builder(this, CHANNEL).setSmallIcon(R.drawable.ic_logo).setContentTitle("AIopter is active").setContentText(getString(R.string.overlay_notification)).setOngoing(true).setContentIntent(PendingIntent.getActivity(this, 0, Intent(this, MainActivity::class.java), PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT)).build()
    ...
    companion object {
        const val ACTION_START_VOICE = "io.aiopter.action.START_VOICE"
        const val ACTION_STOP_ALL = "io.aiopter.action.STOP_ALL"
        private const val CHANNEL = "aiopter_overlay"
